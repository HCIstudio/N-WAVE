...
const executeNextflowWorkflow = async (
  ...
  try {
    ...
    if (useLocalNextflow) {
      ...
      // tags are amd64-only, so on ARM hosts (e.g. Apple Silicon) this runs under
      // emulation instead of failing with "no matching manifest". Native on
      // Intel/AMD. Overridable via NEXTFLOW_PLATFORM.
      ...
      nextflowCmd = `docker run --rm --platform ${nextflowPlatform} --volumes-from ${backendContainerName} -e NXF_LOG_FILE=nextflow/.nextflow.log -w "${dockerMainOutputDir}" nextflow/nextflow:${nextflowVersion} nextflow -log nextflow/.nextflow.log run ./${dockerScriptPath} --outdir results --inputdir inputs --max_cpus ${maxCpus} --max_memory "${maxMemory}" -work-dir nextflow/work`;
    }
